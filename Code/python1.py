...
def two(input):
	input = input.lower()
	count = input.count("bert")
	string = ""
	if count <= 1:
		return ""
	else:
		first = input.find("bert") + 4
		end = input.find("bert", first)
		string = input[first:end]
		return string
...
def five(input):
	...
	for i in range(2, len(record), 4):
		if record[i] == "False":
			if record[i - 2] not in names:
				names.append(record[i-2])
...
def ten(string, int_, char):
	string = string.lower()
	if int_ > len(string):
		return False
	else:
		if string[int_ - 1] == char:
			return True
	return False